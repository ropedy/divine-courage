...
const Logo = props => {
  const [ color, setColor ] = useState(twConfig.theme.extend.colors['dc-red']);

  const onEnter = () => {
    setColor(twConfig.theme.extend.colors['dc-yellow']);
  };

  const onLeave = () => {
    setColor(twConfig.theme.extend.colors['dc-red']);
  };
  ...
  return <svg
    ...
    <rect
      ...
      fill='#404b69'
    />
    <g fill={color} className='transition ease-in-out duration-100'>
